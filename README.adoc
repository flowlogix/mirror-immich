...
FlowLogix MirrorImmich extracts and syncs title, description, tags, GPS location and album information created in Apple macOS Photos and creates a directory that includes symlinks to photos and their extracted metadata in sidecar .xmp files.

This directory can then be rsync'd to Immich external library. These steps are the safest way to continuously sync Apple Photos with Immich, since it preserves all metadata and avoids any potential issues with Immich's internal database.
...
If you find Mirror Immich useful, please consider supporting it on https://github.com/sponsors/flowlogix[GitHub Sponsors] as it will help dedicate more time to it and add more features.

== News
* Aug 15, 2026: We continue to improve MirrorImmich. We added ease of use and error handling improvements.
* Jul 5, 2026: MirrorImmich now fully supports Immich 3.x or later, as well as retaining compatibility with earlier versions of Immich
* Apr 19, 2026: Memory requirements are significantly reduced by processing photos in batches. This allows processing of large libraries on machines with limited memory. See https://github.com/flowlogix/mirror-immich/issues/16[Issue #16] for details.

== Installation
Docker Compose is the recommended installation method.

* Download https://github.com/flowlogix/mirror-immich/archive/refs/tags/2026-01-21.zip[Latest Release Zip]
...
# Optional: set batch size for processing photos (default is 1000)
# PHOTOS_BATCH_SIZE=1000
...
NOTE: New releases are uploaded on Docker Hub and
release date in this repository does not indicate latest update to the Docker image. +
Check https://hub.docker.com/r/flowlogix/mirror-immich[Docker Hub] for the latest image.
...
If anybody is interested in it and/or wants to help me add features (or has any other feedback), please let me know.
...
image:https://static.scarf.sh/a.png?x-pxid=f3156d5e-38e7-40ce-8f3b-e76153eec2f4&page=README.adoc[,0,0]