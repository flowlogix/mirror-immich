...
FlowLogix MirrorImmich extracts metadata created in Apple macOS Photos and creates a directory that includes symlinks to photos and their extracted metadata in sidecar .xmp files.
...
NOTE: Docker needs to have "full dick access" to be able to read Photos library
...
All other solutions extract data from the metadata from the photo and video files themselves. Unfortunately that data is incomplete and does not include manually entered data in Apple Photos on the Mc.
